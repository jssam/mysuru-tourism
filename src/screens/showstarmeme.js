...
import{Text,StyleSheet,FlatList,View,  Alert,Image, ToastAndroid, ActivityIndicator,PermissionsAndroid, } from "react-native";
...
import { setstarmeme } from "../firebase/Firebaseapi";


const showstarmeme =  ({navigation})=>{
...
checkPermission = async (imageur) => {
    ...
    //If Android then ask for runtime permission
 {
  ...
  const createTwoButtonAlert = (meme) =>
    Alert.alert(
      "Become Star",
      "get a chance to be recognized by publicizing your memes by clicking on ok",
      [
        {
          text: "Cancel",
        ...
        { text: "OK", onPress: () => setstarmeme(meme) }
      ],
      { cancelable: true }
    );
...
shareFile=(file_url)=> {
    ...
    .then(async base64Data => {
       ...
       try{ await Share.open({ url: base64Data });
        ...
        setload(false);
        return fs.unlink(imagePath);}catch(err){   setload(false); console.log(err)}
    });
}
...
const memeUrl = navigation.getParam("memeurl");
    ...
    {load ?  <View style={{ flex: 1, alignContent: "center" }}><Image style={{
      ...
      <View style={{ position: "absolute", bottom:160, right: 5 }}>
    ...
    <View style={{ position: "absolute", bottom:90, right: 5 }}>
    ...
    <View style={{ position: "absolute", bottom:20, right: 5 }}>
      <TouchableOpacity onPress={()=>createTwoButtonAlert(`${memeUrl}`)} >
      <Image style={{ height: 60, width: 60 }}
              source={require("../../images/makepublic.png")} />
      </TouchableOpacity>
